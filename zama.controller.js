...
.controller('zamaController', ['$scope', 'zamaFactory', '$cacheFactory', '$locale', function($scope, zamaFactory, $cacheFactory, $locale) {

 	$scope.keys = [];
  $scope.cache = $cacheFactory('zamaCache');
  	
	$scope.put = function(key) {
	    if (angular.isUndefined($scope.cache.get(key))) {
	   		$scope.keys.push(key);
	   		zamaFactory.getData().then(function (responseData) {
